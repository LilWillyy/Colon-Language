...
class Error:
	...
	def as_string(self):
		error_message = f'{self.error_name}: {self.details}, '
		error_message += f'File: {self.pos_start.fn}, line{self.pos_start.ln + 1}'
...
class RTError(Error):
	...
	def as_string(self):
		error_message = self.generate_traceback()
		error_message += f'{self.error_name}: {self.details}, '
		return error_message
	...
	def generate_traceback(self):
		...
		while ctx:
			error_message = f' File {pos.fn}, line {str(pos.ln + 1)}, in {ctx.display_name} \n' + error_message
...
class Position:
	...
	def copy(self):
		return Position(self.indx, self.ln, self.col, self.fn, self.ftxt)
...
class Token:
	...
	# This function defines how each token will be
...
class Lexer:
	...
	def advance(self):
		...
		# Learn compact if statments.
		self.curr_char = self.text[self.pos.indx] if self.pos.indx < len(self.text) else None
...
class Parser:
	...
	def advance(self):
		...
		if self.tok_indx < len(self.tokens):
			self.curr_token = self.tokens[self.tok_indx] 
...
####################################
	...
	def power(self):
		return self.bin_op(self.atom, (TT_PWR, ), self.factor)
	...
	def expr(self):
		...
		if self.curr_token.matches(TT_KEYWORD, 'var'):
			...
			if self.curr_token.type != TT_VARTYPE:
				return res.failure(InvalidSyntaxError(
					self.curr_token.pos_start,
					self.curr_token.pos_end,
			...
			if self.curr_token.type != TT_COLON:
				return res.failure(InvalidSyntaxError(
					self.curr_token.pos_start,
					self.curr_token.pos_end,
					"Expected ':'"
...
class Interpreter:
	...
	def no_visit_method(self, node, context):
		return Exception(f'No visit_{type(node).__name__} defined.')
